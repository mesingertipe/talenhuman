...
const AttendanceMonitoring = ({ user: sessionUser }) => {
    ...
    const fetchSettings = async () => {
        try {
            ...
            // Deduplicate to prioritize the last element (latest from DB) in case of case-casing dupes 
            const uniqueSettingsMap = {};
            attendanceSettings.forEach(s => {
                uniqueSettingsMap[s.key] = s;
            });
            const deduplicatedSettings = Object.values(uniqueSettingsMap);
            ...
            const processedSettings = [...deduplicatedSettings];
            defaults.forEach(def => {
                if (!uniqueSettingsMap[def.key]) {
