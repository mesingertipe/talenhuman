...
const ShiftScheduler = ({ user, tenantSettings, readOnly = false, initialStoreId = null, initialDate = null, forceApprover = false, approvalId = null, onReady = null, suppressOverlay = false }) => {
    ...
    const performOptimization = async () => {
        ...
        try {
            ...
            // 2. REST ASSIGNMENT PASS (Mandatory Rests and empty days)
            employees.forEach(emp => {
                daysInWeek.forEach(day => {
                    const dayStr = day.toDateString();
                    const hasShift = newShifts.some(s => s.employeeId === emp.id && new Date(s.startTime).toDateString() === dayStr);
                    ...
                    if (!hasShift) {
                        ...
                        newShifts.push({
                            ...
                            isFuera: false,
                            isAutoGenerated: true,
                            observation: 'Descanso sugerido por IA'
                        });
                    }
                });
            });
