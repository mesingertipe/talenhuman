...
const ShiftScheduler = ({ user, tenantSettings, readOnly = false, initialStoreId = null, initialDate = null, forceApprover = false, approvalId = null, onReady = null, suppressOverlay = false }) => {
    ...
    const [analysisRuleId, setAnalysisRuleId] = useState(null); // V13.8: Rule segmentation
    ...
    const calculateHourlyNeeds = useCallback((dayDate) => {
        ...
        // V13.8: Rule Metadata for Transparency
        const ruleMetadata = {};

        if (predictiveRules.length === 0) return { needs: {}, volumes: hourlyVolumes, needsByRule: {}, ruleMetadata: {} };
        ...
        predictiveRules.forEach(rule => {
            ...
            if (isRuleActiveToday) {
                ...
                dayHistory.forEach(h => {
                    ...
                    ruleVolumes[hour] = forecastValue;
            ...
            // V13.8 Store Metadata for detailed rule navigation
            ruleMetadata[rule.id] = {
                name: rule.name,
                needs: ruleNeeds,
                volumes: ruleVolumes,
    ...
    const performOptimization = async () => {
        ...
        try {
            // V13.5 ULTIMATE OPTIMIZER: RESPECT PREVIOUS PREDICTIONS
            // We no longer filter out auto-generated shifts here.
            // This allows the user to run optimization incrementally without duplicating what's already there.
            ...
            // V13.8: OpStart/OpEnd are now calculated PER DAY inside the loop below
            
            // TRACKING: Work days per employee
            const workDaysCount = {};
            employees.forEach(e => {
                // Count existing manual + auto work shifts in the week
                workDaysCount[e.id] = newShifts.filter(s => String(s.employeeId).toLowerCase() === String(e.id).toLowerCase() && !s.isDescanso).length;
            });
                ...
                predictiveRules.forEach(rule => {
                    ...
                    for (let h = opStart; h <= opEndHour; h++) {
                        ...
                        while (deficit > 0) {
                            ...
                            // Ensure the shift fits within the operational window of THIS day
                            if (shiftStartHour + dailyHours > opEndHour) {
                                // If it doesn't fit, we start it earlier if possible, but never after closing
                                shiftStartHour = Math.max(opStart, opEndHour - Math.floor(dailyHours));
                            }

                            // Final safety check: if even shifted it starts after closing or before opening, skip
                            if (shiftStartHour >= opEndHour || shiftStartHour < opStart) return;
                            ...
                            newShifts.push({
                                ...
                                observation: 'Opt. por IA (Sugerencia)'
                            });

                            workDaysCount[luckyOne.id] = (workDaysCount[luckyOne.id] || 0) + 1;
                            shiftsAdded++;
                            deficit--;
                        }
                    }
                ...
                if (restsAssigned < mandatoryRests) {
                    daysInWeek.forEach(day => {
                        ...
                        if (!hasAnyShift) {
                            ...
                            startRest.setHours(8, 0, 0, 0);
                            const endRest = new Date(day);
                            endRest.setHours(16, 0, 0, 0);
                            ...
                            newShifts.push({
                                ...
                                storeId: selectedStore,
                                companyId: user.companyId,
                                startTime: startRest.toISOString(),
                                endTime: endRest.toISOString(),
                                status: 0,
                                isDescanso: true,
                                isFuera: false,
                                isAutoGenerated: true,
                                observation: 'Descanso legal'
                            });
                            restsAssigned++;
                        }
                    });
            ...
            const createdCount = shiftsAdded;
        ...
        try {
            ...
            const localizedShifts = shifts
                .filter(s => {
                    ...
                    return empId && isGuid(empId) && s.startTime && s.endTime;
                })
                .map(s => {
                    const sid = s.id || s.Id;
                    const finalId = isGuid(sid) ? sid : null;
                    ...
                    if (finalId) shiftData.id = finalId;
            ...
            console.log("ç²¾è‹±è°ƒåº¦å™¨ (V13.0) - PAYLOAD DEBUG:", payload);
            ...
            if (serverErrors) {
                console.error("ç²¾è‹±è°ƒåº¦å™¨ (V13.0) - VALIDATION ERRORS:", JSON.stringify(serverErrors, null, 2));
            } else {
                console.error("ç²¾è‹±è°ƒåº¦å™¨ (V13.0) - SAVE ERROR DETAIL:", err.response?.data || err);
            }
        ...
        <>
                ...
                <div className="card shadow-[0_40px_100px_rgba(0,0,0,0.12)] bg-white dark:bg-slate-900 border-2 dark:border-slate-800 relative flex-1 flex flex-col" style={{ borderRadius: '48px', overflow: 'hidden' }}>
                    <div className="overflow-auto custom-scrollbar flex-1 elite-scrollable-area">
                            ...
                            <table className="border-collapse" style={{ tableLayout: 'fixed', width: '1120px', borderSpacing: 0, minWidth: '1120px' }}>
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            <div className="flex flex-col gap-1 min-h-[96px] justify-center">
                                                                ...
                                                                {/* Render Programmed Shifts & Associated Attendances */}
                                                                {(() => {
                                                                    const processedShifts = dayShifts.reduce((acc, current) => { 
                                                                        const hasAtt = (attendances || []).some(a => String(a.shiftId) === String(current.id)); 
                                                                        const existing = acc.find(s => s.startTime === current.startTime); 
                                                                        if (!existing) return [...acc, current]; 
                                                                        if (hasAtt && !attendances.some(a => String(a.shiftId) === String(existing.id))) { 
                                                                            return acc.map(s => s.startTime === current.startTime ? current : s); 
                                                                        } 
                                                                        return acc; 
                                                                    }, []);

                                                                    let remainingOrphans = [...dayOrphanAttendances];
                                                                    const shiftElements = processedShifts.map((shift, si) => {
                                                                        let att = attendances.find(a => String(a.shiftId) === String(shift.id) && shift.id);
                                                                        let isVirtual = false;

                                                                        // Logic: If shift has no linked attendance, "consume" an orphan attendance of the same day
                                                                        if (!att && remainingOrphans.length > 0 && !shift.isDescanso) {
                                                                            att = remainingOrphans.shift();
                                                                            isVirtual = true;
                                                                        }
                                                                        
                                                                        let bgColor = '#4f46e5';
                                                                        if (att) {
                                                                            if (att.status === 3 || isVirtual) bgColor = '#f97316';
                                                                        ...
                                                                        const attTime = (att && att.clockIn) ? `${new Date(att.clockIn).getHours().toString().padStart(2, '0')}:${new Date(att.clockIn).getMinutes().toString().padStart(2, '0')}—${att.clockOut ? new Date(att.clockOut).getHours().toString().padStart(2, '0') + ':' + new Date(att.clockOut).getMinutes().toString().padStart(2, '0') : '...'}` : 'S/MARCAR';
                                                                        ...
                                                                        return (
                                                                             <div key={`shift-${si}`} 
                                                                                  ...
                                                                                  onMouseEnter={e => {
                                                                                     const rect = e.currentTarget.getBoundingClientRect();
                                                                                     setHoveredShiftData({ ...shift, att, shiftTime, attTime, isLocked, borderCol: bgColor, isVirtual });
                                                                                     setHoverPos({ x: rect.left + rect.width / 2, y: rect.top });
                                                                                  }}
                                                                             ...
                                                                             >
                                                                                 <div className="flex items-center gap-2 mb-0.5">
                                                                                     ...
                                                                                     {att && (isVirtual ? <Sparkles size={11} className="text-white animate-pulse" /> : <Activity size={12} className="text-white opacity-100 animate-pulse" />)}
                                                                                     <span className="text-[6.5px] font-black uppercase tracking-[0.1em] opacity-80 leading-none">
                                                                                       {viewMode === 'SHIFTS' 
                                                                                        ? (shift.isDescanso ? 'DESC' : shift.isFuera ? 'FUERA' : (isVirtual ? 'TURNO (REAL)' : 'TURNO')) 
                                                                                        : (isVirtual ? 'REAL / EXTRA' : 'MARCACIÓN')}
                                                                                     </span>
                                                                                 </div>
                                                                    ...
                                                                    const orphanElements = remainingOrphans.map((att, ai) => {
                                                                        const attTime = att.clockIn ? `${new Date(att.clockIn).getHours().toString().padStart(2, '0')}:${new Date(att.clockIn).getMinutes().toString().padStart(2, '0')}—${att.clockOut ? new Date(att.clockOut).getHours().toString().padStart(2, '0') + ':' + new Date(att.clockOut).getMinutes().toString().padStart(2, '0') : '...'}` : 'S/MARCAR';
                                                                        let bgColor = '#10b981'; 
                                                                        if (att.status === 1) bgColor = '#eab308'; 
                                                                        if (att.status === 3 || !att.clockOut) bgColor = '#f97316'; 
                                                                    ...
                                                                    return [...shiftElements, ...orphanElements];
                                                                })()}
                                ...
                                <tfoot className="sticky bottom-0 z-[180] shadow-[0_-15px_40px_rgba(0,0,0,0.25)]">
                                    {/* FILA PROGRAMADO */}
                                    <tr className="bg-slate-50 dark:bg-slate-900/80 border-t-2 border-slate-200 dark:border-slate-800">
                                        ...
                                        <td className="p-2 sticky right-0 z-[185] text-center shadow-[-8px_0_20px_rgba(0,0,0,0.2)]" 
                                            style={{ width: '120px', minWidth: '120px', backgroundColor: '#4f46e5', color: '#ffffff', borderLeft: '1px solid rgba(255,255,255,0.1)' }}>
                                            <span className="text-[11px] font-[1000] uppercase">{formatHours(weeklyGlobalTotals.prog)}</span>
                                        </td>
                                    </tr>
                                    ...
                                    <tr className="bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800">
                                        ...
                                        <td className="p-2 sticky right-0 z-[185] text-center shadow-[-8px_0_20px_rgba(0,0,0,0.2)]" 
                                            style={{ width: '120px', minWidth: '120px', backgroundColor: '#10b981', color: '#ffffff', borderLeft: '1px solid rgba(255,255,255,0.1)' }}>
                                            <span className="text-[11px] font-[1000] uppercase">{formatHours(weeklyGlobalTotals.real)}</span>
                                        </td>
                                    </tr>
                                    ...
                                    <tr className="bg-slate-50 dark:bg-slate-900/80 border-t border-slate-200 dark:border-slate-800">
                                        ...
                                        {dailyTotals.map((td, idx) => {
                                            const getEffColor = (val) => {
                                                if (val < 85 || val > 110) return 'bg-rose-100 text-rose-700';
                                                if ((val >= 85 && val < 95) || (val > 105 && val <= 110)) return 'bg-amber-100 text-amber-700';
                                                return 'bg-emerald-100 text-emerald-700';
                                            };
                                            return (
                                                <td key={`eff-${idx}`} className="p-2 border-r dark:border-slate-800 text-center">
                                                    <div className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-black ${getEffColor(td.eff)}`}>
                                                        {td.eff.toFixed(1)}%
                                                    </div>
                                                </td>
                                            );
                                        ...
                                        {(() => {
                                            ...
                                            let finalBg = '#10b981'; // Green (default 95-105)
                                            if (val < 85 || val > 110) finalBg = '#ef4444'; // Red
                                            else if ((val >= 85 && val < 95) || (val > 105 && val <= 110)) finalBg = '#f59e0b'; // Amber
                                            
                                            return (
                                                <td className="p-2 sticky right-0 z-[185] text-center shadow-[-8px_0_20px_rgba(0,0,0,0.2)]" 
                                                    style={{ width: '120px', minWidth: '120px', backgroundColor: finalBg, color: '#ffffff', borderLeft: '1px solid rgba(255,255,255,0.1)' }}>
                                                    <span className="text-[11.5px] font-[1000]">{val.toFixed(1)}%</span>
                                                </td>
                                            );
                                        })()}
                                    </tr>
            ...
            {createPortal(
                                ...
                                <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-100 dark:border-white/5">
                                    <div className="flex flex-col text-left">
                                        <span className={`text-[12px] font-[1000] tracking-tight`} style={{ color: hoveredShiftData.borderCol }}>
                                            {hoveredShiftData.isOrphan ? 'MARCACIÓN SIN TURNO' : 
                                             (hoveredShiftData.isVirtual ? 'MARCACIÓN SIN VINCULAR' : 
                                             (hoveredShiftData.status === 6 ? 'TURNO APROBADO' : 
                                             (hoveredShiftData.status === 7 ? 'TURNO RECHAZADO' : 
                                             (hoveredShiftData.status === 5 ? 'PENDIENTE VALIDACIÓN' : 
                                             (hoveredShiftData.att ? `TURNO ${hoveredShiftData.att.status === 0 ? 'CORRECTO' : (hoveredShiftData.att.status === 3 ? 'INCOMPLETO' : 'DESFASADO')}` : 
                                             (hoveredShiftData.isDescanso ? 'DESCANSO' : 
                                             (hoveredShiftData.isLocked ? 'SIN MARCACIONES' : 'REGISTRADO')))))))}
                                        </span>
                                    </div>
                                    <div className={`w-10 h-10 rounded-2xl flex items-center justify-center shadow-lg`} style={{ backgroundColor: hoveredShiftData.borderCol, color: 'white' }}>
                                        {hoveredShiftData.att ? (hoveredShiftData.att.status === 0 ? <CheckCircle size={22} strokeWidth={3} /> : (hoveredShiftData.att.status === 3 || hoveredShiftData.isVirtual ? <AlertTriangle size={22} strokeWidth={3} /> : <AlertCircle size={22} strokeWidth={3} />)) : <Clock size={20} />}
                                    </div>
                                </div>

                                {/* Content */}
                                <div className="space-y-3">
                                    {(!hoveredShiftData.isOrphan || hoveredShiftData.isVirtual) && (
                                        <div className="flex items-center gap-3">
                                            <Calendar size={14} className="text-indigo-500" />
                                            <div className="flex flex-col text-left">
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="flex flex-col text-left">
                                                <span className="text-[12px] font-[1000] tracking-tight uppercase text-emerald-600">
                                                    {hoveredShiftData.isVirtual ? 'PUNCH SIN VINCULAR' : 'TRABAJO NO PROGRAMADO'}
                                                </span>
                                            </div>
                                        </div>
                                    )}
                ...
                document.body
                    ...
                    {/* ========================================================================= */}
                    {/* ðŸ§  ELITE LAYER: OVERLAYS, MODALS & GLOBAL MONITORS                      */}
                    {/* ========================================================================= */}
                ...
                >
                    ...
                    >
                        ...
                        <div className="p-10 space-y-8 bg-white dark:bg-slate-900">
                            ...
                            {/* V13.8: Rule Navigation Bar */}
                            <div className="flex flex-wrap gap-2 pb-4 border-b border-slate-100 dark:border-white/5">
                                <button 
                                    onClick={() => setAnalysisRuleId(null)}
                                    className={`px-5 py-2.5 rounded-2xl text-[10px] font-black tracking-widest transition-all ${!analysisRuleId ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/30' : 'bg-slate-100 dark:bg-white/5 text-slate-400 dark:text-slate-500 hover:bg-slate-200'}`}
                                >
                                    resumen global
                                </button>
                                {Object.values(selectedCoverageDay.ruleMetadata || {}).map((meta, idx) => (
                                    <button 
                                        key={idx}
                                        onClick={() => setAnalysisRuleId(Object.keys(selectedCoverageDay.ruleMetadata)[idx])}
                                        className={`px-5 py-2.5 rounded-2xl text-[10px] font-black tracking-widest transition-all flex items-center gap-2 ${analysisRuleId === Object.keys(selectedCoverageDay.ruleMetadata)[idx] ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/30' : 'bg-slate-100 dark:bg-white/5 text-slate-400 dark:text-slate-500 hover:bg-slate-200'}`}
                                    >
                                        <div className={`w-2 h-2 rounded-full ${meta.isActive ? 'bg-emerald-400' : 'bg-slate-400'}`}></div>
                            ...
                            <div className="flex flex-col lg:flex-row items-stretch gap-4 bg-slate-50/50 dark:bg-white/5 p-4 rounded-3xl border border-slate-200 dark:border-white/10 backdrop-blur-sm mb-6">
                                 {/* Section 1: Déficit */}
                                <div className="flex-1 flex flex-col justify-center px-6 py-2 border-r border-slate-200 dark:border-white/10 min-w-[140px]">
                                    <p className="text-[10px] font-bold text-rose-500/80 mb-1 leading-none">déficit segmentado</p>
                                    <div className="flex items-baseline gap-2">
                                        ...
                                        <span className="text-[10px] font-bold text-slate-400">colaboradores</span>
                                    </div>
                                </div>

                                {/* Section 2: AI Intelligence */}
                                <div className="flex-[2.5] flex flex-col justify-center px-6 py-2 border-r border-slate-200 dark:border-white/10 min-w-[320px]">
                                        ...
                                        <div className="grid grid-cols-3 gap-4 items-center">
                                            ...
                                            <div className="flex flex-col min-w-[140px]">
                                                <div className="flex justify-between items-center mb-1.5">
                                                    ...
                                                    <span className="text-[9px] font-extrabold text-indigo-400/80">mín. {selectedCoverageDay.ruleMetadata[analysisRuleId].math.opening || 0} — {selectedCoverageDay.ruleMetadata[analysisRuleId].math.closing || 0}</span>
                                                </div>
                                                <div className="flex gap-0.5">
                                                    {['1','2','3','4','5','6','0'].map(d => {
                                                        ...
                                                        return (
                                                            <div key={d} className={`flex-1 h-5 rounded-md flex items-center justify-center text-[9px] font-bold transition-all ${isActive ? 'bg-indigo-600 text-white shadow-sm' : 'bg-slate-200 dark:bg-white/5 text-slate-400 opacity-40'}`}>
                                ...
                                <div className="flex-1 flex flex-col justify-center px-6 py-2 relative overflow-hidden group min-w-[180px]">
                            ...
                            <div className="flex justify-between items-center mb-6">
                                <div className="flex flex-col gap-1">
                                    <h4 className="text-[11px] font-[1000] text-indigo-500 tracking-widest leading-none">semáforo de demanda x hora</h4>
                                    <p className="text-[9px] text-slate-400 font-bold tracking-tight">
                                        {analysisRuleId ? `inspección: ${selectedCoverageDay.ruleMetadata[analysisRuleId].name?.toLowerCase()}` : 'análisis operativo consolidado'}
                                    </p>
                                </div>
                                <div className="flex items-center gap-4 text-[9px] font-black bg-slate-50 dark:bg-slate-800/50 px-4 py-2 rounded-full border border-slate-100 dark:border-white/5">
                                    ...
                                    <div className="flex items-center gap-1.5 text-slate-400"><div className="w-2.5 h-2.5 rounded-full bg-slate-300 dark:bg-slate-700"></div> sin demanda</div>
                                </div>
                            </div>
                                <div className="bg-slate-50/50 dark:bg-slate-800/40 p-10 rounded-[2.5rem] border border-slate-100 dark:border-white/5 shadow-inner">
                                    ...
                                    >
                                        {Array.from({ length: 18 }).map((_, i) => {
                                            ...
                                            if (analysisRuleId) {
                                                const meta = selectedCoverageDay.ruleMetadata[analysisRuleId];
                                                hourNeeds = meta.needs[h] || 0;
                                                hourVolume = meta.volumes[h] || 0;
                                                const rule = predictiveRules.find(r => r.id === analysisRuleId);
                                                ruleProfiles = (rule?.profiles || rule?.Profiles || []).map(p => String(p.profileId || p.ProfileId).toLowerCase());
                                            } else {
                                                hourNeeds = Object.values(selectedCoverageDay.needsByRule || {}).reduce((acc, n) => acc + (n[h] || 0), 0);
                                                hourVolume = selectedCoverageDay.volumes?.[h] || 0;
                                            }
                                            ...
                                            const scheduled = shifts.filter(s => {
                                                ...
                                                if (analysisRuleId && ruleProfiles.length > 0) {
                                                    const emp = employees.find(e => String(e.id).toLowerCase() === String(s.employeeId).toLowerCase());
                                                    if (!emp || !ruleProfiles.includes(String(emp.profileId).toLowerCase())) return false;
                                                }
                            ...
                            {/* V13.8 Bottom Insights Column */}
                            <div className="grid grid-cols-2 gap-8">
                                <div className="space-y-4">
                                     <h4 className="text-[10px] font-black text-emerald-500 uppercase tracking-widest pl-1">Inteligencia Compartida</h4>
                                     <div className="p-8 bg-slate-900 rounded-[2.5rem] text-white relative overflow-hidden shadow-2xl shadow-indigo-500/10">
