...
const ShiftApproval = ({ user }) => {
  ...
  const fetchHistory = async () => {
    try {
      ...
      const res = await api.get(\/ShiftApproval/status?storeId=\&startDate=\\);
  ...
  const renderInspectionMode = () => {
    ...
    return (
      <div className="page-container animate-fade-in" style={{ padding: '0 1.5rem', maxWidth: '1600px', margin: '0 auto' }}>
         <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '2rem 0', borderBottom: \2px solid \\, marginBottom: '2rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
              ...
              <div>
                <h1 style={{ fontSize: '1.5rem', fontWeight: '950', color: activeColors.textMain, margin: 0 }}>Auditoría: \</h1>
                <p style={{ margin: 0, color: activeColors.textMuted, fontSize: '0.85rem' }}>MODO INSPECCIÓN ELITE • PERIODO: \</p>
              </div>
            </div>
         ...
         <div style={{ position: 'relative', overflow: 'hidden', borderRadius: '32px' }}>
            <div style={{ background: activeColors.card, borderRadius: '32px', border: \1.5px solid \\, overflow: 'hidden', padding: '1rem', marginRight: showHistoryDrawer ? '400px' : '0', transition: 'margin 0.4s ease' }}>
            ...
            <div style={{ position: 'absolute', right: showHistoryDrawer ? '0' : '-450px', top: '0', bottom: '0', width: '380px', background: isDarkMode ? '#1e293b' : '#f8fafc', borderRadius: '32px', borderLeft: \2px solid \30\, padding: '28px', transition: 'right 0.4s ease', zIndex: 50, overflowY: 'auto' }}>
               ...
               {fetchingHistory ? <p>Cargando...</p> : history.map((log, idx) => (
                 <div key={idx} style={{ marginBottom: '20px', padding: '15px', borderRadius: '20px', background: activeColors.card, border: \1px solid \\ }}>
                    <div style={{ fontSize: '0.7rem', fontWeight: '900', color: log.action === 'Approved' ? '#10b981' : '#ef4444', marginBottom: '5px' }}>\</div>
                    <p style={{ fontSize: '0.85rem', fontWeight: '700', margin: '0 0 10px 0' }}>"\"</p>
                    <div style={{ fontSize: '0.7rem', color: activeColors.textMuted }}>\ • \</div>
                 </div>
               ))}
            </div>
         </div>
        ...
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '3rem', alignItems: 'center' }}>
            ...
            {activeTab === 'PENDING' && (
              <div style={{ display: 'flex', gap: '12px' }}>
                <button disabled={selectedKeys.length === 0 || processing} onClick={() => setShowCommentModal(true)} style={{ padding: '12px 24px', borderRadius: '15px', border: 'none', background: '#fee2e2', color: '#ef4444', fontWeight: '800', cursor: 'pointer', opacity: selectedKeys.length === 0 ? 0.5 : 1 }}>Rechazar (\)</button>
          ...
          <div style={{ display: 'flex', gap: '10px', marginBottom: '2.5rem' }}>
            {['PENDING', 'APPROVED', 'REJECTED'].map(tID => (
              <button key={tID} onClick={() => setActiveTab(tID)} style={{ padding: '10px 20px', borderRadius: '12px', border: 'none', fontWeight: '900', background: activeTab === tID ? activeColors.accent : activeColors.card, color: activeTab === tID ? 'white' : activeColors.textMuted, cursor: 'pointer' }}>\</button>
            ))}
          </div>
          <div style={{ background: activeColors.card, borderRadius: '25px', border: \1.5px solid \\, overflow: 'hidden' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {loading ? <tr><td colSpan="4" style={{ padding: '50px', textAlign: 'center' }}>Cargando datos...</td></tr> : filteredStores.map(store => (
                  <tr key={\\-\\} style={{ borderBottom: \1px solid \\ }}>
                    {activeTab === 'PENDING' && <td style={{ padding: '15px' }}><input type="checkbox" checked={selectedKeys.includes(\\-\\)} onChange={() => handleSelectStore(store)} /></td>}
                    <td style={{ padding: '15px' }}><span style={{ fontWeight: '800' }}>\</span></td>
      ...
      {showApprovalModal && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(4px)' }}>
          <div style={{ background: activeColors.card, padding: '30px', borderRadius: '25px', width: '400px', textAlign: 'center' }}>
            ...
            <p>Se aprobarán \ mallas semanales.</p>
            <div style={{ display: 'flex', gap: '10px', marginTop: '20px' }}>
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 10000000, display: 'flex', alignItems: 'center', justifyContent: 'center', background: isDarkMode ? 'rgba(15, 23, 42, 0.9)' : 'rgba(30,30,60,0.8)', backdropFilter: 'blur(10px)' }}>
           <div style={{ background: isDarkMode ? '#1e293b' : 'white', padding: '40px', borderRadius: '40px', textAlign: 'center', width: '380px', boxShadow: '0 30px 60px rgba(0,0,0,0.5)' }}>
              ...
              <div style={{ height: '5px', background: '#f1f5f9', borderRadius: '10px', overflow: 'hidden' }}>
                <div style={{ height: '100%', width: \\%\, background: '#4f46e5', transition: 'width 0.6s ease' }} />
              </div>
           </div>
        </div>,
        document.body
