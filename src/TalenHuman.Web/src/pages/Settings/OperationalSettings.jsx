                ...
                <div className="bg-white dark:bg-slate-800 p-8 md:p-12 rounded-[40px] border border-slate-100 dark:border-slate-700/50 shadow-sm">
                    ...
                    <div className="space-y-8">
                        ...
                        <ToggleItem 
                            ...
                            description="Envío de alerta vibrante a los empleados solo cuando el horario ha sido aprobado."
...
const ModeCard = ({ active, onClick, icon, title, description, badge, color }) => {
    ...
    return (
        ...
        >
            {badge && (
                <span className="absolute -top-3 left-8 px-4 py-1.5 bg-indigo-600 text-[10px] font-black tracking-tighter text-white rounded-full skew-x-[-12deg]">
                    {badge}
                </span>
            )}
            <div className="flex flex-col gap-6">
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-transform duration-500 group-hover:scale-110 ${active ? 'bg-current text-white' : 'bg-slate-100 dark:bg-slate-700 text-slate-400'}`}>
                    {icon}
                </div>
                <div>
                    <h3 className={`text-lg font-black tracking-tight mb-2 ${active ? 'text-slate-900 dark:text-white' : 'text-slate-500'}`}>{title}</h3>
                    <p className={`text-[11px] font-bold leading-relaxed ${active ? 'text-slate-600 dark:text-slate-400' : 'text-slate-400'}`}>{description}</p>
                </div>
    ...
    return (
        <div className="flex items-center justify-between gap-8 group">
            <div className="flex items-center gap-6">
                <div className={`p-4 rounded-2xl transition-colors duration-300 ${active ? 'bg-indigo-600 text-white shadow-lg' : 'bg-slate-100 dark:bg-slate-700 text-slate-400'}`}>
                    {icon}
                </div>
                <div>
