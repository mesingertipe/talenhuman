...
const MobileLayout = ({ children, activePage, setPage, user, onLogout, version }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col text-slate-900 dark:text-slate-100 overscroll-none overflow-hidden">
      ...
      <header className="fixed top-0 left-0 right-0 z-[100] bg-indigo-900 text-white pt-safe pb-6 px-6 rounded-b-[2.5rem] shadow-2xl shadow-indigo-900/40">
        {/* Sub-header for branding info */}
        <div className="flex items-center justify-between pt-4">
           <div className="flex items-center gap-3">
              <div className="w-9 h-9 bg-white/10 backdrop-blur-md rounded-2xl flex items-center justify-center border border-white/20">
                 <span className="font-black text-lg italic tracking-tighter text-white">TH</span>
              </div>
              <div>
                 <h1 className="text-xs font-black tracking-widest leading-tight uppercase text-blue-200/60 transition-all">TalenHuman</h1>
              </div>
           </div>
